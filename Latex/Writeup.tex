...
% Title page
\title{\Huge\bfseries\sffamily QMC Option Pricing}
\subtitle{\large A repository on the implementation of QMC option pricing techniques under Black Scholes.}
...
This benchmark function is implemented inside the \texttt{Benchmark.py} file, under the name \path{european_call_cf(S0, K, T, r, sigma)}.
...
\begin{itemize}
\item{High bias → model is too simple (underfitting)}
\item{Low bias → model closely captures the true relationship}
\item{High variance → model is too complex (overfitting)}
\item{Low variance → model is more stable across datasets}
\item{Lower RMSE → better predictive accuracy (has the same units as the predicted magnitude)}
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/AT_SS.png}
\caption{Antithetic variates vs Stratified sampling}
\end{figure}
...
\subsection{Halton (Van der Corput)}
...
\begin{enumerate}
    \item{A \texttt{skip} parameter (default 100) discards initial points, which can improve uniformity by avoiding early transient behaviour.}
    \item {Indices $i = \text{skip}, \text{skip}+1, \dots, \text{skip}+m \times n - 1$ are generated.}
    \item {For each index, the number of trailing zeros (\texttt{tz}) is computed using \texttt{(\~i) \& (i+1)}. The position of the rightmost zero bit determines which direction number to use.}
    \item {The direction numbers \texttt{dirs} are precomputed as \path{1 << (bits - c)} where \path{c = floor(log2(tz)) + 1}. This corresponds to a simple set of primitive polynomials (the Sobol sequence in base 2).}
    \item {A cumulative XOR over the direction numbers yields the integer Sobol values \texttt{x}.}
    \item {These integers are scaled to the unit interval by dividing by $2^{\text{bits}} = 2^{32}$.}
\end{enumerate}
...
These methods are implemented as \texttt{Kronecker(m,n)}, \texttt{Halton(m,n)} and \texttt{Sobol(m,n)}. When compared to the default random number generation for numpy, a significant improvement in variance can be found, at a higher computational cost. As detailed in \texttt{Sequence\_testing.ipynb}, the improvement in variance is of aproximately two orders of magnitude. When comparing variance to computational cost (independent of number of samples), only this implementation of Sobol matches the random number generation. Nonetheless, in the next section, numpy implementations for these sequences will be used, so computational cost will be improved on further.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/Rand_LDS.png}
\caption{Computation time vs Variance}
\end{figure}

\newpage
\section{Low discrepancy sequences applied to Black-Scholes}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/Kron_metrics.png}
\caption{Kronecker sampling metrics}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/Halton_metrics.png}
\caption{Halton sampling metrics}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/Sobol_metrics.png}
\caption{Sobol sampling metrics}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/MC_IS_rel.png}
\caption{Monte Carlo vs Importance Sampling relative variance}
\end{figure}
...
\section{Cross Entropy importance sampling}
...
\begin{enumerate}
    \item{Set $a_0=0$}
    \item {Compute $w_i=H(Z_i)L(Z_i)$}
    \item {Compute $a_{k+1}=\frac{\sum w_iZ_i}{\sum w_i}$}
    \item {Repeat using the new mean shift}
\end{enumerate}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/IS_CE_rel.png}
\caption{Cross-entropy vs Importance sampling relative variance}
\end{figure}

\chapter{Path dependent options}
\section{Monte Carlo implementation}
...
This path dependent model is implemented using Monte Carlo simulation as a baseline, as \texttt{path\_dependent\_mc(S0, K, T, r, rho, timesteps, n, exp)}. When compared to the regular Monte Carlo Black Scholes implementation, it can be noted that variance is much higher, due to the added randomness of volatility following a stochastic process. The assymptotic rate, however, is the same.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/MC_PD.png}
\caption{Monte Carlo Black Scholes vs Path Dependent volatility}
\end{figure}

\section{Antithethic variates}

The simplest improvement to Monte Carlo, also for path dependent options, is the use of antithetic variates. By simulating a path and its opposite, the computational cost of number generation is reduced, and variance is also decreased. It is implemented as \texttt{path\_dependent\_at(S0, K, T, r, rho, timesteps, n, exp)}.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/PD_AT.png}
\caption{Path dependent MC vs Antithetic variates}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/PD_AT_time.png}
\caption{Path dependent MC vs Antithetic variates}
\end{figure}
...
\begin{itemize}
    ...
    \item{$\beta$ is chosen to minimize variance}
\end{itemize}
...
This is implemented in the repository as \texttt{path\_dependent\_cv(S0, K, T, r, rho, timesteps, n, exp)}. When compared to the previous Monte Carlo implementation, the reduction in variance of the estimator can be seen.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../Assets/MC_CV.png}
\caption{Path dependent MC vs Control Variates}
\end{figure}

\chapter{Final findings}
...
\begin{table}[H]
\centering
\caption{Normalized efficiencies for moderate estimates}
\begin{tabularx}{0.8\textwidth}{lXr}
\toprule
\textbf{Method} & \textbf{Normalized Efficiency}\\
\midrule
...
\begin{table}[H]
\centering
\caption{Normalized efficiencies for deep ITM estimates}
\begin{tabularx}{0.8\textwidth}{lXr}
\toprule
\textbf{Method} & \textbf{Normalized Efficiency}\\
\midrule
